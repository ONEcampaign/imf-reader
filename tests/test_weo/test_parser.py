...
class TestSDMXParser:
    ...
    def test_parse_xml(self):
        ...
        root = ET.Element('message:StructureSpecificData', attrib={'xmlns:ss': 'http://www.sdmx.org/resources/sdmxml/schemas/v2_1/data/structurespecific', 'xmlns:footer': 'http://www.sdmx.org/resources/sdmxml/schemas/v2_1/message/footer', 'xmlns:ns1': 'urn:sdmx:org.sdmx.infomodel.datastructure.DataStructure=IMF:WEO_PUB(1.0):ObsLevelDim:TIME_PERIOD', 'xmlns:message': 'http://www.sdmx.org/resources/sdmxml/schemas/v2_1/message', 'xmlns:common': 'http://www.sdmx.org/resources/sdmxml/schemas/v2_1/common', 'xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'xmlns:xml': 'http://www.w3.org/XML/1998/namespace'})
        header = ET.SubElement(root, 'message:Header')
        dataset = ET.SubElement(root, 'message:DataSet', attrib={'ss:dataScope': 'DataStructure', 'xsi:type': 'ns1:DataSetType', 'ss:structureRef': 'IMF_WEO_PUB_1_0'})
        series = ET.SubElement(dataset, 'Series', attrib={'UNIT': 'B', 'CONCEPT': 'NGDP_D', 'REF_AREA': '111', 'FREQ': 'A', 'LASTACTUALDATE': '2023', 'SCALE': '1', 'NOTES': 'See notes for:  Gross domestic product, constant prices (National currency) Gross domestic product, current prices (National currency).'})
        obs = ET.SubElement(series, 'Obs', attrib={'TIME_PERIOD': '1980', 'OBS_VALUE': '39.372'})
        ...
        expected_df = pd.DataFrame([{'UNIT': 'B', 'CONCEPT': 'NGDP_D', 'REF_AREA': '111', 'FREQ': 'A', 'LASTACTUALDATE': '2023', 'SCALE': '1', 'NOTES': 'See notes for:  Gross domestic product, constant prices (National currency) Gross domestic product, current prices (National currency).', 'TIME_PERIOD': '1980', 'OBS_VALUE': '39.372'}])
    ...
    def test_lookup_schema_element(self):
        ...
        simpleType = ET.SubElement(root, '{%s}simpleType' % ns['xs'], attrib={'name': 'IMF.CL_WEO_UNIT.1.0'})
        restriction = ET.SubElement(simpleType, '{%s}restriction' % ns['xs'], attrib={'base': 'xs:string'})
        enumeration = ET.SubElement(restriction, '{%s}enumeration' % ns['xs'], attrib={'value': 'A'})
    ...
    @patch('imf_reader.weo.parser.SDMXParser.lookup_schema_element', return_value={'A': 'Current international dollar'})
    def test_add_label_columns(self, mock_lookup):
        """Test for add_label_columns method."""

        # Create a mock DataFrame
        data_df = pd.DataFrame([{'UNIT': 'A', 'CONCEPT': 'NGDP_D', 'REF_AREA': '111', 'FREQ': 'A', 'LASTACTUALDATE': '2023', 'SCALE': '1', 'NOTES': 'See notes for:  Gross domestic product, constant prices (National currency) Gross domestic product, current prices (National currency).', 'TIME_PERIOD': '1980', 'OBS_VALUE': '39.372'}])
    ...
    def test_check_folder(self, mock_zip):
        ...
        with pytest.raises(UnexpectedFileError, match = "There should be exactly one xml file in the folder"):
        ...
        with pytest.raises(UnexpectedFileError, match="There should be exactly one xsd file in the folder"):
            SDMXParser.check_folder(mock_zip)
